...
class GenericTeamCommands
{
    ...
    fun sethome(@Sender player: Player)
    {
        ...
        if (!team.subLeaders.contains(player.uniqueId) && team.leader != player.uniqueId)
        {
    ...
    fun accept(@Sender sender: Player, @Name("team") teamString: String)
    {
        ...
        if (!tryingToAcceptTeam.pendingInvites.contains(sender.uniqueId))
        {
            sender.sendMessage(Chat.format("&cThis team has not invited you"))
            return
        }

        tryingToAcceptTeam.members.add(sender.uniqueId)
        tryingToAcceptTeam.pendingInvites.remove(sender.uniqueId)
    ...
    fun invite(@Sender sender: Player, @Name("target") target: String)
    {
        ...
        if (!team.subLeaders.contains(sender.uniqueId) || team.leader != sender.uniqueId)
        {
        ...
        team.save()
        team.sendGlobalTeamMessage("&a" + hcfplayer.name + " &ehas been invited to the team")
    }


    @Command(value = ["team who", "f who", "t who", "f i", "team info"])
    ...
    fun kik(@Sender sender: Player, @Name("target") target: String)
    {
        ...
        if (Bukkit.getPlayer(player.uuid) != null)
        {
            Bukkit.getPlayer(player.uuid).sendMessage(Chat.format("&cYou were kicked from the faction!"))
        }
    }
    ...
    fun kick(@Sender sender: Player, @Name("target") target: HCFPlayer)
    {
        ...
        if (Bukkit.getPlayer(target.uuid) != null)
        {
            ...
            Bukkit.getPlayer(target.uuid).sendMessage(Chat.format("&cYou have been kicked from your faction"))
        }
    ...
    fun disbandTeam(@Sender player: Player)
    {
        ...
        if (teamForPlayer.leader != player.uniqueId)
        {
